...
- **Dungeon crawlers loot like it's 1999 — and the bards sing.** Clearing a room now means picking the corpses OVER: gold, gems, scrolls, and reagents all ride home, and a **magic drop** is the run's payday — the crawler blurts it out on the spot ("no way. its magic") and the find hits the event journal, so the bank crowd retells it for days ("{he} pulled a halberd of vanquishing off a corpse down Shame??"). And the era's richest farming build finally plays like itself: a crawler with real **Provocation** works its lute constantly — its own attacker gets a well-played tune and turns on the nearest other monster while the bard slips out of the fight, and idle pairs get set brawling so there's always a corpse to pick over. Sour notes included, skill-checked.
