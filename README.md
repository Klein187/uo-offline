...
- **Tamers fight with their pets — by player rules, no shortcuts.** A Tamer heading out to hunt **claims its pet from the stables first**: the trip detours to the counter, "vendor claim", and the beast walks out of the pens — laddered to what its Animal Taming can genuinely hold, from a Novice's timber wolf up to the Grandmaster's **nightmare, white wyrm, or the odd dragon**. Every order is typed out loud, exactly the commands players hammered: **"all kill"** on each fresh target, **"all stay"** before kneeling to vet-bandage the pet, **"all follow me"** to heel. And **feeding is real**: the tamer carries raw ribs, feeds the pet at the engine's own loyalty rate (munch sounds and all), and restocks ribs on supply errands like any other consumable — a tamer that runs dry watches loyalty slide until the pet **goes wild for real** and is loose in the world. Pets follow their tamer through town between hunts — the nightmare parade at the bank is back.
