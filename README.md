...
- **Bots move more smoothly.** The engine picks a moving creature's run or walk animation from its actual step pace now, instead of from a flag the caller passed in. A bot stepping at run speed while flagged as walking backed the client's animation queue up until it snapped forward. Bots set the flag from their own step timer, so it matches the speed they are really moving at.
...
**Answer their WTB shouts.** "WTB GM hally" used to be noise. Tell whoever shouted it that you have one and they come over and haggle, the same negotiation a WTS shout starts, just from the other end. Only a bot with the coin to back its shout can be answered.
