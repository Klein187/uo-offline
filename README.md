...
Every time you click **UO Offline** it asks: **Play by myself**, **Host for friends**, or **Join a friend**. Tick "don't ask again" if you always want the same one; `friends.bat ask` in the install folder brings the question back (`./friends.sh ask` on Linux and the Steam Deck).

**Hosting** keeps everything as it is and lets friends into your world while your game is running. Windows asks once to open port 2593 in its firewall, and the launcher shows the address to give out (so does `friends.bat`). Same house: your LAN address. Anywhere else: put [Tailscale](https://tailscale.com) on both PCs and use the `100.x.y.z` address it gives you; nothing is opened to the internet and no router changes are needed. Friends' characters live in your save.

**Joining** asks for your friend's address, a name and a password, then connects the game there. Your account is created on their world the first time you log in. If their game is not running you get a message saying so. Full steps and fixes for "cannot connect" are in [docs/FRIENDS.md](docs/FRIENDS.md).
...
**Your guild, with bots in it.** Recruit a bot at your guildstone and it joins on its own a few seconds later. A guilded bot is a permanent resident: it keeps living its life, but the session manager never logs it out, the boot purge and the regenerations leave it alone, and it comes back after a restart with your tag on. Guild chat is delivered to the guild (a T2A server normally drops it), and a guild member asking for a group in guild chat brings up to four guild bots: they answer in guild chat, recall to the asker if they have the magery, regs or a scroll, walk if they are already close, say so if they cannot come, and take the party invite when they arrive. While you are online the guild bots also use the channel themselves: random chatter, hellos answered, "what is everyone doing" answered from each bot's real activity ("heading to vesper provisioner", "in Despise L1", "with a group at Britain"), and now and then one asks for a group; its guildmates recall to it and it leads them into a dungeon as a guild crew, unless you answer "yeah" first, in which case they come to you.

**Thieves pick pockets.** Thief-class bots carry the NPC Thieves Guild card, and most of them also wear the tag of The Thieves Guild [TG], a player guild that only thieves roll into. They work the places people stand around with full packs: bank plazas, taverns, shop doors. One picks a mark, walks up, or hides and stealths up, snoops the pack and steals the best light item with the real Stealing skill, so gold comes off in the amounts the skill allows and a magic weapon can walk off. Most thieves only rob marks standing away from the town NPCs, because a criminal flagged within eight tiles of one gets a guard on the spot. The bold ones gamble. The lift itself always reveals the thief, so after a clean one it often hides again and slips off. Underground a thief never crawls: it works the floor it landed on hidden, moving room to room along the waypoints, robs adventurers between fights, treats a monster as something to hide from, and leaves by the stairs after a lift. A noticed thief runs, hides once it has a gap, and waits out the flag; the fighters nearby draw on it the way they draw on any gray. After every successful lift the thief goes to a bank and puts exactly what it took into its box.
