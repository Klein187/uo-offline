...
- **Recall is really cast now.** Bot travel runs the genuine RecallSpell end to end: a marked recall rune pulled from the pack and targeted at cursor time, real black pearl / bloodmoss / mandrake burned from the pack (fizzles eat reagents too, as they always did), real recall scrolls consumed on success, and the era's true skill checks — scrolls read two circles easier, which is exactly why shaky casters carried them. Fizzles get re-cast like a player mashing the macro; a trip that won't take gets walked instead ("cant get this spell off"). No more arrival smoke — just Kal Ort Por and the double recall chime.
