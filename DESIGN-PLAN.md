...
- **Phase 1 (FIRST):** place your OWN spawns + filters + generate in-game.
  Self-contained, immediately useful, establishes the data model + filter UI.
- **Phase 2 (later):** live population view — show actual current entities at
  real positions, same filters, density heatmap. Needs a live-game-query
  endpoint in serve_map.py (the novel piece — everything else reads static
  JSON; live positions need a running-game query).
- **Phase 3 (later):** ingest Nerun's UOClassic.map so all ~1700 existing
  spawns appear in the editor and can be moved/edited/deleted. Biggest piece:
  parse Nerun's .map format, round-trip edits back out, decide whether editing
  Nerun's file or adopting its spawns into our own system.
